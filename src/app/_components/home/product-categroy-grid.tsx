...
export async function ProductCategoryGrid() {
	...
	return (
		<section className="mx-auto max-w-7xl px-4 py-14 sm:px-6">
			<h2 className="text-center font-extrabold text-3xl text-[#14163A]">Our products</h2>
				...
				{categories.map((category) => (
					...
					>
						<span className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-white/70">
							{category.imageUrl ? (
								<Image src={category.imageUrl} alt="" width={28} height={28} className="h-7 w-7 object-contain" />
							) : (
								<span className="text-lg">✺</span>
							)}
						</span>
						<span className="text-base uppercase tracking-wide">{category.name}</span>
					</Link>
				))}
			</div>
		</section>
