...
export function FloatingActions({ whatsappNumber }: FloatingActionsProps) {
	...
	const cartBarIsOnScreen =
		pathname.startsWith("/estimate") && cart.itemCount > 0;

	return (
		<div
			className={cn(
				"pointer-events-none fixed inset-x-0 z-30 flex items-end justify-between px-4 sm:px-6 lg:bottom-6",
				cartBarIsOnScreen ? "bottom-24" : "bottom-4",
			)}
		>
			{waLink ? (
				<a
					aria-label="Chat with us on WhatsApp"
					className="pointer-events-auto flex h-14 w-14 items-center justify-center rounded-full bg-[var(--brand-whatsapp)] text-white shadow-lg transition hover:scale-105 hover:shadow-xl"
					href={waLink}
			...
			<div className="pointer-events-auto flex flex-col items-end gap-3">
				<a
					className="flex items-center gap-2 rounded-full bg-accent px-4 py-2.5 font-bold text-accent-foreground text-sm shadow-lg transition hover:scale-105 hover:shadow-xl"
				...
				<Link
					className="flex items-center gap-2 rounded-full bg-primary px-5 py-3 font-bold text-primary-foreground text-sm shadow-lg transition hover:scale-105 hover:shadow-xl"
					href="/estimate"
				>
					Estimate now
				</Link>
