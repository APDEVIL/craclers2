...
export default async function AdminDashboardPage() {
	const [pendingOrders, allOrders, products, activeCarts, leads] =
		await Promise.all([
	...
	const stats = [
		{
			...
			sub: `₹${pendingRevenue.toFixed(2)} potential`,
			href: "/admin/orders",
			icon: Inbox,
		},
		{
				...
				{stats.map((stat) => {
					...
					return (
						...
						>
							...
							<p className="mt-4 font-extrabold text-2xl text-[#14163A]">
								{stat.value}
							</p>
							<p className="font-semibold text-[#14163A]/70 text-sm">
								{stat.label}
							</p>
			...
			<div className="grid gap-6 lg:grid-cols-2">
				<div className="rounded-lg border border-[#14163A]/10 bg-white p-5">
					<div className="flex items-center justify-between">
						...
						<Link
							className="font-medium text-[#C8202F] text-sm hover:underline"
							href="/admin/orders"
						>
							View all
						</Link>
					...
					<div className="mt-4 space-y-1">
						...
						{allOrders.length === 0 ? (
							<p className="py-6 text-center text-[#14163A]/50 text-sm">
								No estimates yet.
							</p>
						) : null}
					</div>
					...
					<div className="mt-4 space-y-1">
						{leads.slice(0, 5).map((lead) => (
							...
							>
								...
								<a
									className="font-medium text-[#C8202F] hover:underline"
